...
#define BC_PTR (reinterpret_cast<uint16_t*>(m_regs + 0))
#define BC_VAL (*reinterpret_cast<uint16_t*>(m_regs + 0))

#define DE_PTR (reinterpret_cast<uint16_t*>(m_regs + 2))
#define DE_VAL (*reinterpret_cast<uint16_t*>(m_regs + 2))

#define HL_PTR (reinterpret_cast<uint16_t*>(m_regs + 4))
#define HL_VAL (*reinterpret_cast<uint16_t*>(m_regs + 4))
...
public:
    ...
    void CpuStep(bool& stop_signal);

    inline Memory* GetMemory() const {return m_memory;}

private:

    void decodeAndExecute(uint8_t opcode, bool& stop_signal, unsigned int& m_cycles_count);
    ...
    inline void bitSetTo(uint8_t& number, uint8_t n, bool x) {
        number = (number & ~((uint8_t)1 << n)) | ((uint8_t)x << n);
    }

    inline bool getBitFrom(uint8_t val, uint8_t k) {
        return (val & ( 1 << k )) >> k;
    }
